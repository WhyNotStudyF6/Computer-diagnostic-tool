...
namespace PcCheckTool
{
    ...
    internal sealed partial class MainForm : Form
    {
        ...
        private const string AppVersion = "V2.1.21";
        ...
        private const string DefaultOfficeCommunicationTestUrl = "任务流示例";
        private const string DefaultOfficeCommunicationTestPayload = "示例：默认会顺序执行 任务流示例 目录里的 10 个示例文件。\r\n你也可以改成自己的说明、批注或临时文本。";
        private const string DefaultOfficeCommunicationWindowTitle = "任务流窗口";
        private const string DefaultOfficeCommunicationWindowDescription = "用于按顺序运行指定目录中的可执行程序，并展示每一步的执行结果与耗时。";
        private const int DefaultOfficeCommunicationStepDelayMs = 300;
        private const string DefaultOfficeCommunicationResultFileName = "office_comm_result.txt";
        private const string DefaultOfficeCustomMenuText = "发送按键串";
        private const string DefaultOfficeCustomActionRelativePath = "内置:发送按键串";
        private const string LegacyOfficeCustomActionRelativePath = @"自定义右键示例\01_打开说明.cmd";
        private const string LegacyOfficeCustomActionDescriptionPath = @"自定义右键示例\自定义右键说明.txt";
        ...
        private ToolStripSeparator _officeCustomActionSeparator;
        ...
        public MainForm(StartupMode startupMode)
        {
            ...
            EnsureTaskFlowAndCustomActionExamples();
            ...
            _toolTip.SetToolTip(_settingsButton, "打开设置，配置 WiFi、电池阈值和工具路径");
        ...
        private void InitializeOfficeOperationMenu()
        {
            ...
            _officeCustomActionSeparator = new ToolStripSeparator();
            RefreshOfficeCustomOperationMenuItem();
        }
        ...
        private Button CreateCompactModeButton(string text, EventHandler onClick, string toolTipText, string name = null, Button sourceButton = null)
        {
            ...
            {
                button.MouseUp += CompactThemeButtonMouseUp;
                _toolTip.SetToolTip(button, "左击切换明亮/黑暗主题；右击打开系统个性化");
            }
        ...
        private void CompactThemeButtonMouseUp(object sender, MouseEventArgs e)
        {
            if (e == null || e.Button != MouseButtons.Right)
            {
                return;
        ...
        private string BuildOfficeActionTooltipText()
        {
            ...
            return string.Format(
                "当前操作：{0}。点击后只会把密钥写入目标文件并复制到剪贴板，不会自动打开 Office。\r\n剩余密钥：{1} 个\r\n操作计数：{2}\r\nOffice 当前状态：{3}\r\n右击此按钮可切换复制/剪切/删除密钥文件首行，也可打开任务流窗口或执行自定义右键功能。",
        ...
        private bool TryExecuteCustomActionTarget(string pathOrUri)
        {
            if (string.IsNullOrWhiteSpace(pathOrUri))
            {
                return false;
            ...
            if (IsShellUri(trimmed) || !File.Exists(trimmed))
            {
                return TryStartTarget(trimmed);
            }
                ...
                switch (extension)
                {
                    case ".ps1":
                        startInfo = new ProcessStartInfo
                        {
                            ...
                            Arguments = string.Format("-NoProfile -ExecutionPolicy Bypass -File \"{0}\"", trimmed),
                            UseShellExecute = false,
                    ...
                    case ".cmd":
                        ...
                        {
                            FileName = "cmd.exe",
                            Arguments = string.Format("/c \"\"{0}\"\"", trimmed),
                            UseShellExecute = false,
                            CreateNoWindow = false
                        };
                        break;
        ...
        private bool StartCustomActionTarget(string pathOrUri)
        {
            if (TryExecuteCustomActionTarget(pathOrUri))
            {
                return true;
            }

            ShowErrorMessage(string.Format("执行或打开失败：{0}", pathOrUri));
            return false;
        }

        private static bool IsBuiltInOfficeCustomAction(string configuredPath)
        {
            return string.Equals((configuredPath ?? string.Empty).Trim(), DefaultOfficeCustomActionRelativePath, StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void ShowSettingsDialog()
        {
            using (var dialog = new Form())
            {
                ...
                var officeCommTab = new TabPage("任务流");
                ...
                officeCommTab.AutoScrollMinSize = new Size(820, 1180);
                ...
                tabs.TabPages.Add(officeCommTab);
                ...
                var officeCommWindowTitleLabel = new Label { Text = "窗口标题", Location = new Point(18, 30), Size = new Size(120, 22) };
                var officeCommWindowTitleBox = new TextBox
                {
                    Location = new Point(18, 56),
                    Size = new Size(642, 28),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCommunicationWindowTitle) ? DefaultOfficeCommunicationWindowTitle : _settings.OfficeCommunicationWindowTitle
                };
                var officeCommDescriptionLabel = new Label { Text = "窗口说明(可多行)", Location = new Point(18, 92), Size = new Size(160, 22) };
                var officeCommDescriptionBox = new TextBox
                {
                    Location = new Point(18, 118),
                    Size = new Size(642, 62),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCommunicationWindowDescription) ? DefaultOfficeCommunicationWindowDescription : _settings.OfficeCommunicationWindowDescription,
                    Multiline = true,
                    ScrollBars = ScrollBars.Vertical,
                    AcceptsReturn = true
                };
                officeCommDescriptionBox.KeyDown += (_, e) =>
                {
                    if (e == null || e.KeyCode != Keys.Enter || !e.Shift)
                    {
                        return;
                    ...
                    e.SuppressKeyPress = true;
                    e.Handled = true;
                    var selectionStart = officeCommDescriptionBox.SelectionStart;
                    var newLine = Environment.NewLine;
                    officeCommDescriptionBox.SelectedText = newLine;
                    officeCommDescriptionBox.SelectionStart = selectionStart + newLine.Length;
                };
                var officeCommUrlLabel = new Label { Text = "执行目录(按顺序运行)", Location = new Point(18, 190), Size = new Size(180, 22) };
                ...
                var officeCommPayloadLabel = new Label { Text = "附加文字/备注", Location = new Point(18, 254), Size = new Size(160, 22) };
                ...
                var officeCommDelayLabel = new Label { Text = "步骤间等待(ms)", Location = new Point(18, 364), Size = new Size(140, 22) };
                var officeCommDelayBox = new NumericUpDown
                {
                    ...
                    Maximum = 30000,
                    Increment = 100,
                    Value = Math.Max(0, _settings.OfficeCommunicationStepDelayMs)
                };
                ...
                {
                    Text = "这里配置任务流窗口。右击 Office 按钮可打开窗口；它会按执行目录里的可执行文件数量生成一排步骤，并按顺序执行。\r\n默认示例目录是 任务流示例，可直接试跑。整个窗口只负责顺序执行与结果展示，不内置激活、联网或代填逻辑。",
                    Location = new Point(24, 52),
                ...
                var officeCommGroup = new GroupBox
                {
                    ...
                    Size = new Size(680, 516)
                };
                var officeCommTip = new TextBox
                {
                    Text = "窗口里保留少量常用输入；设置页里保留更完整的默认配置。步骤数量由执行目录中的可执行文件数决定；成功会变绿勾，失败会变红叉，并记录执行耗时。\r\n推荐直接放 .exe / .bat / .cmd / .ps1；若放 txt、md 或链接，也会按系统默认方式打开。",
                ...
                {
                    officeCommWindowTitleLabel, officeCommWindowTitleBox, officeCommDescriptionLabel, officeCommDescriptionBox,
                    officeCommUrlLabel, officeCommUrlBox, officeCommPayloadLabel, officeCommPayloadBox, officeCommDelayLabel, officeCommDelayBox, officeCommTip
                });
                var officeCustomGroup = new GroupBox
                {
                    Text = "自定义右键列表",
                    Location = new Point(24, 664),
                    Size = new Size(680, 488)
                };
                var officeCustomKeyTitleConfigLabel = new Label { Text = "按键串列表名字", Location = new Point(18, 30), Size = new Size(120, 22) };
                var officeCustomKeyTitleBox = new TextBox
                {
                    Location = new Point(146, 27),
                    Size = new Size(514, 28),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCustomKeyListTitle) ? "自定义按键串" : _settings.OfficeCustomKeyListTitle.Trim()
                };
                var officeCustomKeyLabel = new Label { Text = string.Format("{0}(可多个，空行分隔)", string.IsNullOrWhiteSpace(_settings.OfficeCustomKeyListTitle) ? "自定义按键串" : _settings.OfficeCustomKeyListTitle.Trim()), Location = new Point(18, 64), Size = new Size(320, 22) };
                var officeCustomKeyBox = new TextBox
                {
                    Location = new Point(18, 90),
                    Size = new Size(642, 120),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCustomKeyActionsText) ? BuildDefaultOfficeCustomKeyActionsText() : _settings.OfficeCustomKeyActionsText,
                    Multiline = true,
                    ScrollBars = ScrollBars.Vertical,
                    AcceptsReturn = true
                };
                officeCustomKeyTitleBox.TextChanged += (_, __) =>
                {
                    var normalizedTitle = string.IsNullOrWhiteSpace(officeCustomKeyTitleBox.Text) ? "自定义按键串" : officeCustomKeyTitleBox.Text.Trim();
                    officeCustomKeyLabel.Text = normalizedTitle + "(可多个，空行分隔)";
                };
                officeCustomKeyBox.KeyDown += (_, e) =>
                {
                    if (e == null || e.KeyCode != Keys.Enter || !e.Shift)
                    {
                        return;
                    ...
                    e.SuppressKeyPress = true;
                    e.Handled = true;
                    var selectionStart = officeCustomKeyBox.SelectionStart;
                    var newLine = Environment.NewLine;
                    officeCustomKeyBox.SelectedText = newLine;
                    officeCustomKeyBox.SelectionStart = selectionStart + newLine.Length;
                };
                var officeCustomKeyTip = new TextBox
                {
                    Text = "格式：每段第一行是菜单名称，下面几行写 按键:/文本:/停顿:；段与段之间空一行。右键 Office 时会把这些菜单项直接列出来。",
                    Location = new Point(18, 216),
                    Size = new Size(642, 40),
                    BorderStyle = BorderStyle.None,
                    ReadOnly = true,
                    Multiline = true,
                    BackColor = officeCommTab.BackColor,
                ...
                var officeCustomFileTitleConfigLabel = new Label { Text = "文件列表名字", Location = new Point(18, 266), Size = new Size(120, 22) };
                var officeCustomFileTitleBox = new TextBox
                {
                    Location = new Point(146, 263),
                    Size = new Size(514, 28),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCustomFileListTitle) ? "自定义可执行文件/文档" : _settings.OfficeCustomFileListTitle.Trim()
                };
                var officeCustomFileLabel = new Label { Text = string.Format("{0}(可多个，一行一个)", string.IsNullOrWhiteSpace(_settings.OfficeCustomFileListTitle) ? "自定义可执行文件/文档" : _settings.OfficeCustomFileListTitle.Trim()), Location = new Point(18, 298), Size = new Size(360, 22) };
                var officeCustomFileBox = new TextBox
                {
                    Location = new Point(18, 324),
                    Size = new Size(642, 88),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCustomFileActionsText) ? BuildDefaultOfficeCustomFileActionsText() : _settings.OfficeCustomFileActionsText,
                    Multiline = true,
                    ScrollBars = ScrollBars.Vertical,
                    AcceptsReturn = true
                };
                officeCustomFileTitleBox.TextChanged += (_, __) =>
                {
                    var normalizedTitle = string.IsNullOrWhiteSpace(officeCustomFileTitleBox.Text) ? "自定义可执行文件/文档" : officeCustomFileTitleBox.Text.Trim();
                    officeCustomFileLabel.Text = normalizedTitle + "(可多个，一行一个)";
                };
                officeCustomFileBox.KeyDown += (_, e) =>
                {
                    if (e == null || e.KeyCode != Keys.Enter || !e.Shift)
                    {
                        return;
                    ...
                    e.SuppressKeyPress = true;
                    e.Handled = true;
                    var selectionStart = officeCustomFileBox.SelectionStart;
                    var newLine = Environment.NewLine;
                    officeCustomFileBox.SelectedText = newLine;
                    officeCustomFileBox.SelectionStart = selectionStart + newLine.Length;
                };
                var officeCustomFileExampleLabel = new Label { Text = "默认示例", Location = new Point(18, 418), Size = new Size(120, 22) };
                var officeCustomFileExampleBox = new TextBox
                {
                    Location = new Point(18, 442),
                    Size = new Size(642, 20),
                    ReadOnly = true,
                    BorderStyle = BorderStyle.None,
                    BackColor = officeCommTab.BackColor,
                    ForeColor = Color.DimGray,
                    Text = BuildDefaultOfficeCustomFileActionsText()
                };
                officeCustomGroup.Controls.AddRange(new Control[]
                {
                    officeCustomKeyTitleConfigLabel, officeCustomKeyTitleBox, officeCustomKeyLabel, officeCustomKeyBox, officeCustomKeyTip,
                    officeCustomFileTitleConfigLabel, officeCustomFileTitleBox, officeCustomFileLabel, officeCustomFileBox, officeCustomFileExampleLabel, officeCustomFileExampleBox
                });
                officeCommTab.Controls.AddRange(new Control[]
                {
                    officeCommTitleLabel, officeCommTabTip, officeCommGroup, officeCustomGroup
                });
                ...
                {
                    Text = "这里专门设置右击 Office 时“发送按键串”要发送的内容。支持三类：按键: / 文本: / 停顿:。\r\n任一设置处回车保存全部；多行输入框里 Shift+回车换行。触发条件可多行同时写；任意一个条件满足就触发。若要每个条件对应不同指令串，可在后续按键串里用“条件:xxx”开新规则。",
                    Location = new Point(24, 52),
                    Size = new Size(680, 52),
                    BorderStyle = BorderStyle.None,
                    ReadOnly = true,
                ...
                {
                    Text = "启用 Office 后续条件触发动作",
                    Location = new Point(18, 32),
                    Size = new Size(240, 24),
                    Checked = _settings.OfficePostActionEnabled
                };
                var officePostConditionLabel = new Label { Text = "触发条件", Location = new Point(18, 72), Size = new Size(120, 22) };
                var officePostConditionBox = new TextBox
                {
                    Location = new Point(18, 98),
                    Size = new Size(642, 68),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficePostActionCondition) ? DefaultOfficePostActionCondition : _settings.OfficePostActionCondition,
                    Multiline = true,
                    ScrollBars = ScrollBars.Vertical,
                    AcceptsReturn = true
                };
                ...
                {
                    Text = "可多行同时写，例如：关闭激活弹窗后 / 用户按C后 / 延时500后。程序会同时监测；任意一个条件满足就触发。",
                    Location = new Point(18, 172),
                    Size = new Size(642, 34),
                    ForeColor = Color.DimGray
                };
                var officePostSequenceLabel = new Label { Text = "后续按键串", Location = new Point(18, 214), Size = new Size(120, 22) };
                ...
                {
                    Text = "可直接写一组共享指令；也可写多组规则，例如：条件:关闭激活弹窗后 + 多行按键；条件:用户按C后 + 另一组多行按键。推荐每行带类型：按键:/文本:/停顿:。",
                ...
                {
                    Text = "“发送按键”“追加文本”“停顿”都会按带类型的一行写入上面的“后续按键串”；如果要新增一组规则，可手动先写一行“条件:用户按C后”再继续往下追加。",
                ...
                {
                    officePostTitleLabel, officePostTabTip, officePostGroup, officePostBuilderGroup, officePostAhkGroup
                });
                ...
                okButton.Click += (_, __) =>
                {
                    ...
                    _settings.OfficeCommunicationWindowTitle = string.IsNullOrWhiteSpace(officeCommWindowTitleBox.Text) ? DefaultOfficeCommunicationWindowTitle : officeCommWindowTitleBox.Text.Trim();
                    _settings.OfficeCommunicationWindowDescription = string.IsNullOrWhiteSpace(officeCommDescriptionBox.Text) ? DefaultOfficeCommunicationWindowDescription : officeCommDescriptionBox.Text.Trim();
                    ...
                    _settings.OfficeCommunicationStepDelayMs = (int)officeCommDelayBox.Value;
                    _settings.OfficeCustomKeyListTitle = string.IsNullOrWhiteSpace(officeCustomKeyTitleBox.Text) ? "自定义按键串" : officeCustomKeyTitleBox.Text.Trim();
                    _settings.OfficeCustomFileListTitle = string.IsNullOrWhiteSpace(officeCustomFileTitleBox.Text) ? "自定义可执行文件/文档" : officeCustomFileTitleBox.Text.Trim();
                    _settings.OfficeCustomKeyActionsText = string.IsNullOrWhiteSpace(officeCustomKeyBox.Text) ? BuildDefaultOfficeCustomKeyActionsText() : officeCustomKeyBox.Text.Trim();
                    _settings.OfficeCustomFileActionsText = string.IsNullOrWhiteSpace(officeCustomFileBox.Text) ? BuildDefaultOfficeCustomFileActionsText() : officeCustomFileBox.Text.Trim();
                    _settings.OfficeCustomActionsText = _settings.OfficeCustomFileActionsText;
                    var firstKeyAction = GetConfiguredOfficeCustomKeyActions().FirstOrDefault();
                    if (firstKeyAction != null)
                    {
                        _settings.OfficeCustomActionMenuText = firstKeyAction.Text;
                        _settings.OfficeCustomActionFilePath = DefaultOfficeCustomActionRelativePath;
                        _settings.OfficePostActionSequence = string.IsNullOrWhiteSpace(firstKeyAction.Sequence) ? DefaultOfficePostActionSequence : firstKeyAction.Sequence.Trim();
                    }
                    _settings.OfficePostActionEnabled = officePostActionEnabledBox.Checked;
                    _settings.OfficePostActionCondition = string.IsNullOrWhiteSpace(officePostConditionBox.Text) ? DefaultOfficePostActionCondition : officePostConditionBox.Text.Trim();
        ...
        private void EnsureTaskFlowAndCustomActionExamples()
        {
            try
            {
                var appDir = GetApplicationDirectory();

                var taskFlowDir = Path.Combine(appDir, "任务流示例");
                Directory.CreateDirectory(taskFlowDir);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "01_准备.cmd"),
                    "@echo off\r\necho [01_PREPARE] READY\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "02_成功.cmd"),
                    "@echo off\r\necho [02_SUCCESS] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "03_失败示例.cmd"),
                    "@echo off\r\necho [03_FAIL] FAIL SAMPLE\r\nexit /b 1\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "04_继续成功.cmd"),
                    "@echo off\r\necho [04_CONTINUE] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "05_再次成功.cmd"),
                    "@echo off\r\necho [05_AGAIN] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "06_检查完成.cmd"),
                    "@echo off\r\necho [06_CHECK] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "07_写入说明.cmd"),
                    "@echo off\r\necho [07_NOTE] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "08_补充成功.cmd"),
                    "@echo off\r\necho [08_EXTRA] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "09_第二失败示例.cmd"),
                    "@echo off\r\necho [09_FAIL] FAIL SAMPLE\r\nexit /b 1\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowDir, "10_结束成功.cmd"),
                    "@echo off\r\necho [10_FINISH] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);

                var taskFlowAllSuccessDir = Path.Combine(appDir, "任务流示例_全成功");
                Directory.CreateDirectory(taskFlowAllSuccessDir);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "01_准备.cmd"),
                    "@echo off\r\necho [01_PREPARE] READY\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "02_成功.cmd"),
                    "@echo off\r\necho [02_SUCCESS] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "03_完成.cmd"),
                    "@echo off\r\necho [03_FINISH] ALL PASS\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "04_环境检查.cmd"),
                    "@echo off\r\necho [04_ENV] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "05_发送文本.cmd"),
                    "@echo off\r\necho [05_TEXT] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "06_等待返回.cmd"),
                    "@echo off\r\necho [06_WAIT] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "07_处理结果.cmd"),
                    "@echo off\r\necho [07_RESULT] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "08_写入日志.cmd"),
                    "@echo off\r\necho [08_LOG] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "09_收尾检查.cmd"),
                    "@echo off\r\necho [09_TAIL] OK\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(taskFlowAllSuccessDir, "10_全部成功.cmd"),
                    "@echo off\r\necho [10_DONE] ALL PASS\r\nexit /b 0\r\n",
                    Encoding.ASCII);

                var customDir = Path.Combine(appDir, "自定义右键示例");
                Directory.CreateDirectory(customDir);
                EnsureExampleFile(
                    Path.Combine(customDir, "01_打开说明.cmd"),
                    "@echo off\r\nstart \"\" notepad.exe \"%~dp0自定义右键说明.txt\"\r\nexit /b 0\r\n",
                    Encoding.ASCII);
                EnsureExampleFile(
                    Path.Combine(customDir, "自定义右键说明.txt"),
                    "电脑检测工具 V2.1.6 右键自定义示例\r\n\r\n当前默认示例：\r\n- 自定义右键说明.txt\r\n  作用：直接打开这份说明文件，避免 cmd 窗口一闪而过，更方便确认右键功能是否正常。\r\n\r\n附带脚本示例：\r\n- 01_打开说明.cmd\r\n  作用：演示通过 cmd 间接打开说明文件。\r\n\r\n推荐可配置类型：\r\n- .exe：直接启动程序\r\n- .bat / .cmd：运行批处理\r\n- .ps1：运行 PowerShell 脚本\r\n- .txt / .md：用系统默认程序打开说明文件\r\n- http/https 链接：用默认浏览器打开\r\n\r\n配置建议：\r\n1. 先在设置-任务流-自定义右键功能里把菜单名称改成你想显示的文字。\r\n2. 再把“功能文件路径”指向你自己的 exe、cmd、ps1 或说明文件。\r\n3. 路径可写相对路径，软件会优先按软件目录或 U 盘目录尝试解析。\r\n",
        ...
        private static void EnsureExampleFile(string path, string content, Encoding encoding)
        {
            if (string.IsNullOrWhiteSpace(path) || File.Exists(path))
            {
                return;
        ...
        private enum OfficeCustomActionKind
        {
            KeySequence,
            Target
        }

        private sealed class OfficeCustomActionEntry
        {
            public string Text;
            public string Target;
            public string Sequence;
            public OfficeCustomActionKind Kind;
        }

        private static string BuildDefaultOfficeCustomKeyActionsText()
        {
            return "发送按键串\r\n"
                + "按键:{Alt}\r\n"
                + "停顿:250\r\n"
        ...
        private static string BuildDefaultOfficeCustomFileActionsText()
        {
            return "打开右键示例|自定义右键示例\\自定义右键说明.txt";
        }

        private static IEnumerable<string> SplitTextBlocks(string raw)
        {
            var normalized = (raw ?? string.Empty).Replace("\r\n", "\n").Replace('\r', '\n');
            var blocks = normalized.Split(new[] { "\n\n" }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var block in blocks)
            {
                var trimmed = (block ?? string.Empty).Trim();
                if (!string.IsNullOrWhiteSpace(trimmed))
                {
        ...
        private List<OfficeCustomActionEntry> GetConfiguredOfficeCustomKeyActions()
        {
            var raw = _settings.OfficeCustomKeyActionsText ?? string.Empty;
            var result = new List<OfficeCustomActionEntry>();
            foreach (var block in SplitTextBlocks(raw))
            {
                var lines = block
                    .Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries)
                ...
                var text = lines[0];
                var sequence = string.Join(Environment.NewLine, lines.Skip(1));
                if (string.IsNullOrWhiteSpace(text) || string.IsNullOrWhiteSpace(sequence))
                {
                    continue;
                }

                result.Add(new OfficeCustomActionEntry
                {
                    Text = text,
                    Sequence = sequence,
                    Kind = OfficeCustomActionKind.KeySequence
                });
            }

            if (result.Count == 0)
            {
                var defaultBlock = BuildDefaultOfficeCustomKeyActionsText();
                var lines = defaultBlock.Split(new[] { "\r\n" }, StringSplitOptions.None);
                result.Add(new OfficeCustomActionEntry
                {
                    Text = lines[0].Trim(),
                    Sequence = string.Join(Environment.NewLine, lines.Skip(1)),
                    Kind = OfficeCustomActionKind.KeySequence
                });
            }
        ...
        private List<OfficeCustomActionEntry> GetConfiguredOfficeCustomFileActions()
        {
            var raw = (_settings.OfficeCustomFileActionsText ?? string.Empty).Replace("\r\n", "\n");
            var result = new List<OfficeCustomActionEntry>();
            foreach (var line in raw.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                var text = trimmed.Substring(0, separatorIndex).Trim();
                var target = trimmed.Substring(separatorIndex + 1).Trim();
                ...
                result.Add(new OfficeCustomActionEntry
                {
                    Text = text,
                    Target = target,
                    Kind = OfficeCustomActionKind.Target
                });
            }

            if (result.Count == 0)
            {
                var fallbackLine = BuildDefaultOfficeCustomFileActionsText();
                var separatorIndex = fallbackLine.IndexOf('|');
                result.Add(new OfficeCustomActionEntry
                {
                    Text = fallbackLine.Substring(0, separatorIndex).Trim(),
                    Target = fallbackLine.Substring(separatorIndex + 1).Trim(),
                    Kind = OfficeCustomActionKind.Target
                });
            }
        ...
        private List<OfficeCustomActionEntry> GetConfiguredOfficeCustomActions()
        {
            var result = new List<OfficeCustomActionEntry>();
            result.AddRange(GetConfiguredOfficeCustomKeyActions());
            result.AddRange(GetConfiguredOfficeCustomFileActions());
            return result;
        }

        private string ResolveOfficeCustomActionPath(string configuredPath)
        {
            var normalized = string.IsNullOrWhiteSpace(configuredPath)
                ? DefaultOfficeCustomActionRelativePath
                : configuredPath.Trim();
            if (IsBuiltInOfficeCustomAction(normalized))
            {
                return DefaultOfficeCustomActionRelativePath;
            }

            return BuildPortableCandidates(normalized)
                .FirstOrDefault(candidate => !string.IsNullOrWhiteSpace(candidate) && (IsShellUri(candidate) || File.Exists(candidate)));
        }

        private void RefreshOfficeCustomOperationMenuItem()
        {
            if (_officeOperationMenu == null)
            {
                return;
            ...
            _officeOperationMenu.Items.Clear();
            ...
            _officeOperationMenu.Items.Add("打开任务流窗口", null, (_, __) => ShowOfficeCommunicationShellDialog());

            var customActions = GetConfiguredOfficeCustomActions();
            if (customActions.Count > 0)
            {
                _officeOperationMenu.Items.Add(_officeCustomActionSeparator);
                foreach (var entry in customActions)
                {
                    var captured = entry;
                    _officeOperationMenu.Items.Add(new ToolStripMenuItem(captured.Text, null, (_, __) => ExecuteCustomOfficeOperation(captured)));
                }
                _officeOperationMenu.Items.Add(new ToolStripSeparator());
            }
        ...
        private void ExecuteCustomOfficeOperation(OfficeCustomActionEntry entry)
        {
            if (entry == null)
            {
                return;
            ...
            if (entry.Kind == OfficeCustomActionKind.KeySequence)
            {
                ExecuteManualOfficeSequence(entry.Sequence, entry.Text);
                return;
            }

            var customPath = ResolveOfficeCustomActionPath(entry.Target);
            if (string.IsNullOrWhiteSpace(customPath))
            {
                ShowErrorMessage("未找到自定义 Office 右键功能文件，请检查设置里的“自定义可执行文件列表”，或先使用默认示例。", "Office");
                return;
            }

            StartCustomActionTarget(customPath);
        }

        private void ExecuteManualOfficeSequence(string sequenceText, string actionName)
        {
            if (string.IsNullOrWhiteSpace(sequenceText))
            {
                ShowErrorMessage(string.Format("“{0}”当前未设置按键串，请先到设置里的“任务流”页填写。", string.IsNullOrWhiteSpace(actionName) ? "发送按键串" : actionName), "Office");
                return;
            }
        ...
        private void ShowOfficeCommunicationTestDialog()
        {
            using (var dialog = new Form())
            {
                ...
                dialog.AutoScaleMode = AutoScaleMode.None;
                dialog.KeyPreview = true;
                dialog.ClientSize = new Size(760, 640);
                dialog.Font = Font;
                ...
                var titleLabel = new Label { Text = "页面标题", Location = new Point(18, 48), Size = new Size(120, 22) };
                var titleBox = new TextBox
                {
                    Location = new Point(18, 74),
                    Size = new Size(720, 28),
                    Text = string.IsNullOrWhiteSpace(_settings.OfficeCommunicationWindowTitle)
                        ? DefaultOfficeCommunicationWindowTitle
                        : _settings.OfficeCommunicationWindowTitle
                };
                titleBox.TextChanged += (_, __) =>
                {
                    var titleText = string.IsNullOrWhiteSpace(titleBox.Text) ? DefaultOfficeCommunicationWindowTitle : titleBox.Text.Trim();
                    dialog.Text = titleText;
                    headingLabel.Text = titleText;
                };
                var descriptionLabel = new Label { Text = "页面说明", Location = new Point(18, 112), Size = new Size(120, 22) };
                ...
                var urlLabel = new Label { Text = "文本通讯 URL", Location = new Point(18, 212), Size = new Size(120, 22) };
                ...
                var payloadLabel = new Label { Text = "发送文本", Location = new Point(18, 276), Size = new Size(120, 22) };
                ...
                var responseLabel = new Label { Text = "返回文本/备注", Location = new Point(18, 398), Size = new Size(160, 22) };
                var responseBox = new RichTextBox
                {
                    Location = new Point(18, 424),
                    Size = new Size(720, 94),
                    ReadOnly = false,
                    ShortcutsEnabled = true,
                    DetectUrls = false,
                    BorderStyle = BorderStyle.FixedSingle,
                ...
                var progressLabel = new Label { Text = "当前进度 0/5", Location = new Point(18, 528), Size = new Size(720, 22) };
                ...
                Action<int> setProgress = step =>
                {
                    var normalizedStep = Math.Max(progressBar.Minimum, Math.Min(progressBar.Maximum, step));
                    progressBar.Value = normalizedStep;
                    progressLabel.Text = string.Format("当前进度 {0}/{1}", normalizedStep, progressBar.Maximum);
                };
                var previousStepButton = new Button { Text = "上一步", Location = new Point(18, 584), Size = new Size(88, 32) };
                var nextStepButton = new Button { Text = "下一步", Location = new Point(112, 584), Size = new Size(88, 32) };
                var resetProgressButton = new Button { Text = "重置进度", Location = new Point(206, 584), Size = new Size(96, 32) };
                var saveDefaultsButton = new Button { Text = "保存为默认", Location = new Point(308, 584), Size = new Size(96, 32) };
                var copyPayloadButton = new Button { Text = "复制发送文本", Location = new Point(410, 584), Size = new Size(104, 32) };
                var copyAllButton = new Button { Text = "复制全部", Location = new Point(520, 584), Size = new Size(96, 32) };
                var closeButton = new Button { Text = "关闭", Location = new Point(642, 584), Size = new Size(96, 32) };
                ...
                Action saveDefaults = delegate
                {
                    ...
                    _settings.OfficeCommunicationTestUrl = (urlBox.Text ?? string.Empty).Trim();
                    _settings.OfficeCommunicationTestPayload = payloadBox.Text ?? string.Empty;
                    _settings.Save();
                };

                previousStepButton.Click += (_, __) => setProgress(progressBar.Value - 1);
                nextStepButton.Click += (_, __) => setProgress(progressBar.Value + 1);
                resetProgressButton.Click += (_, __) => setProgress(0);
                saveDefaultsButton.Click += (_, __) => saveDefaults();
                copyPayloadButton.Click += (_, __) =>
                {
                    if (string.IsNullOrWhiteSpace(payloadBox.Text))
                    {
                        return;
                    }
                    ...
                    try
                    {
                        CopyTextToClipboard(payloadBox.Text);
                    }
                    catch (Exception ex)
                    {
                        ShowErrorMessage("复制发送文本失败：" + ex.Message, "文本通讯测试");
                    }
                };
                copyAllButton.Click += (_, __) =>
                {
                    ...
                    {
                        ShowErrorMessage("复制通讯窗内容失败：" + ex.Message, "文本通讯测试");
                    }
                };
                ...
                dialog.FormClosing += (_, __) => saveDefaults();
                setProgress(0);

                dialog.Controls.AddRange(new Control[]
                {
                    headingLabel, titleLabel, titleBox, descriptionLabel, descriptionBox,
                    urlLabel, urlBox, payloadLabel, payloadBox, responseLabel, responseBox,
                    progressLabel, progressBar, previousStepButton, nextStepButton, resetProgressButton,
                    saveDefaultsButton, copyPayloadButton, copyAllButton, closeButton
                });
        ...
        private bool TryPostOfficeCommunicationPayload(Uri uri, string payloadText, out string responseText, out string responseSummary)
        {
            ...
            return TrySendOfficeCommunicationRequest(
                uri,
                "POST",
    ...
    internal sealed class AppSettings
    {
        ...
        public string OfficeCommunicationWindowTitle = "任务流窗口";
        public string OfficeCommunicationWindowDescription = "用于按顺序运行指定目录中的可执行程序，并展示每一步的执行结果与耗时。";
        public string OfficeCommunicationTestUrl = "任务流示例";
        public string OfficeCommunicationTestPayload = "示例：默认会顺序执行 任务流示例 目录里的 01_准备.cmd、02_成功.cmd、03_失败示例.cmd。\r\n你也可以改成自己的说明、批注或临时文本。";
        public int OfficeCommunicationStepDelayMs = 300;
        public string OfficeCustomActionMenuText = "发送按键串";
        public string OfficeCustomActionFilePath = "内置:发送按键串";
        public string OfficeCustomKeyListTitle = "自定义按键串";
        public string OfficeCustomFileListTitle = "自定义可执行文件/文档";
        public string OfficeCustomKeyActionsText =
            "发送按键串\r\n" +
            "按键:{Alt}\r\n" +
            "停顿:250\r\n" +
            "按键:D\r\n" +
            ...
            "停顿:500\r\n" +
            "按键:{Enter}";
        public string OfficeCustomFileActionsText = "打开右键示例|自定义右键示例\\自定义右键说明.txt";
        public string OfficeCustomActionsText = "发送按键串|内置:发送按键串\r\n打开右键示例|自定义右键示例\\自定义右键说明.txt";
        ...
        private static string SettingsPath
        {
            ...
            {
                var baseDir = AppDomain.CurrentDomain.BaseDirectory;
                return Path.Combine(baseDir, "settings_v21.ini");
            }
        }
        ...
        public static AppSettings Load()
        {
            ...
            foreach (var rawLine in File.ReadAllLines(sourceSettingsPath, Encoding.UTF8))
            {
                ...
                if (line.StartsWith("office_comm_window_title=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCommunicationWindowTitle = Unescape(line.Substring("office_comm_window_title=".Length));
                    continue;
                }

                if (line.StartsWith("office_comm_window_description=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCommunicationWindowDescription = Unescape(line.Substring("office_comm_window_description=".Length));
                    continue;
                }

                if (line.StartsWith("office_comm_step_delay=", StringComparison.OrdinalIgnoreCase))
                {
                    int value;
                    if (int.TryParse(line.Substring("office_comm_step_delay=".Length), out value))
                    {
                        settings.OfficeCommunicationStepDelayMs = Math.Max(0, value);
                    }
                    continue;
                }

                if (line.StartsWith("office_custom_action_text=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomActionMenuText = Unescape(line.Substring("office_custom_action_text=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_action_path=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomActionFilePath = Unescape(line.Substring("office_custom_action_path=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_actions_text=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomActionsText = Unescape(line.Substring("office_custom_actions_text=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_key_list_title=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomKeyListTitle = Unescape(line.Substring("office_custom_key_list_title=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_file_list_title=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomFileListTitle = Unescape(line.Substring("office_custom_file_list_title=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_key_actions_text=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomKeyActionsText = Unescape(line.Substring("office_custom_key_actions_text=".Length));
                    continue;
                }

                if (line.StartsWith("office_custom_file_actions_text=", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomFileActionsText = Unescape(line.Substring("office_custom_file_actions_text=".Length));
                    continue;
                }
            ...
            var customMenuText = (settings.OfficeCustomActionMenuText ?? string.Empty).Trim();
            var customActionPath = (settings.OfficeCustomActionFilePath ?? string.Empty).Trim();
            if (string.Equals(customActionPath, @"自定义右键示例\01_打开说明.cmd", StringComparison.OrdinalIgnoreCase)
                || string.Equals(customActionPath, @"自定义右键示例\自定义右键说明.txt", StringComparison.OrdinalIgnoreCase))
            {
                settings.OfficeCustomActionFilePath = "内置:发送按键串";
                if (string.IsNullOrWhiteSpace(customMenuText) || string.Equals(customMenuText, "打开右键示例", StringComparison.OrdinalIgnoreCase))
                {
                    settings.OfficeCustomActionMenuText = "发送按键串";
                }
            }

            if (string.IsNullOrWhiteSpace(settings.OfficeCustomActionsText))
            {
                settings.OfficeCustomActionsText = (settings.OfficeCustomActionMenuText ?? "发送按键串").Trim()
                    + "|"
                    + (string.IsNullOrWhiteSpace(settings.OfficeCustomActionFilePath) ? "内置:发送按键串" : settings.OfficeCustomActionFilePath.Trim());
                if (!settings.OfficeCustomActionsText.Contains("打开右键示例|"))
                {
                    settings.OfficeCustomActionsText += "\r\n打开右键示例|自定义右键示例\\自定义右键说明.txt";
                }
            }

            if (string.IsNullOrWhiteSpace(settings.OfficeCustomKeyActionsText) || string.IsNullOrWhiteSpace(settings.OfficeCustomFileActionsText))
            {
                var migratedKeyBlocks = new List<string>();
                var migratedFileLines = new List<string>();
                var rawActions = (settings.OfficeCustomActionsText ?? string.Empty).Replace("\r\n", "\n");
                foreach (var line in rawActions.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    var trimmed = (line ?? string.Empty).Trim();
                    var separatorIndex = trimmed.IndexOf('|');
                    if (separatorIndex <= 0 || separatorIndex >= trimmed.Length - 1)
                    ...
                    if (string.Equals(target, "内置:发送按键串", StringComparison.OrdinalIgnoreCase))
                    {
                        var sequence = string.IsNullOrWhiteSpace(settings.OfficePostActionSequence)
                            ? new AppSettings().OfficePostActionSequence
                            : settings.OfficePostActionSequence.Trim();
                        migratedKeyBlocks.Add(text + "\r\n" + sequence);
                    }
                    else
                    {
                        migratedFileLines.Add(text + "|" + target);
                    }
                }

                if (string.IsNullOrWhiteSpace(settings.OfficeCustomKeyActionsText))
                {
                    settings.OfficeCustomKeyActionsText = migratedKeyBlocks.Count > 0
                        ? string.Join("\r\n\r\n", migratedKeyBlocks)
                        : new AppSettings().OfficeCustomKeyActionsText;
                }

                if (string.IsNullOrWhiteSpace(settings.OfficeCustomFileActionsText))
                {
                    settings.OfficeCustomFileActionsText = migratedFileLines.Count > 0
                        ? string.Join("\r\n", migratedFileLines)
                        : new AppSettings().OfficeCustomFileActionsText;
                }
            }

            if (string.IsNullOrWhiteSpace(settings.OfficeCustomKeyListTitle))
            {
                settings.OfficeCustomKeyListTitle = "自定义按键串";
            }

            if (string.IsNullOrWhiteSpace(settings.OfficeCustomFileListTitle))
            {
                settings.OfficeCustomFileListTitle = "自定义可执行文件/文档";
            }
        ...
        public void Save()
        {
            ...
            lines.Add("office_comm_window_title=" + Escape(OfficeCommunicationWindowTitle));
            lines.Add("office_comm_window_description=" + Escape(OfficeCommunicationWindowDescription));
            ...
            lines.Add("office_comm_step_delay=" + OfficeCommunicationStepDelayMs);
            lines.Add("office_custom_action_text=" + Escape(OfficeCustomActionMenuText));
            lines.Add("office_custom_action_path=" + Escape(OfficeCustomActionFilePath));
            lines.Add("office_custom_actions_text=" + Escape(OfficeCustomActionsText));
            lines.Add("office_custom_key_list_title=" + Escape(OfficeCustomKeyListTitle));
            lines.Add("office_custom_file_list_title=" + Escape(OfficeCustomFileListTitle));
            lines.Add("office_custom_key_actions_text=" + Escape(OfficeCustomKeyActionsText));
            lines.Add("office_custom_file_actions_text=" + Escape(OfficeCustomFileActionsText));
