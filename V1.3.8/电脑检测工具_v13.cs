...
namespace PcCheckTool
{
    ...
    internal sealed class MainForm : Form
    {
        ...
        private const string AppVersion = "V1.3.8";
        ...
        private double _memoryDisplayTotalBytes;
        private double _diskDisplayTotalBytes;
        ...
        {
            var memoryText = _memoryDisplayTotalBytes > 0
                ? FormatCompactCapacityFromBytes(_memoryDisplayTotalBytes)
                : ExtractCompactCapacityText(_memoryLabel == null ? string.Empty : _memoryLabel.Text, false);
            var diskText = _diskDisplayTotalBytes > 0
                ? FormatCompactCapacityFromBytes(_diskDisplayTotalBytes)
                : ExtractCompactCapacityText(_diskLabel == null ? string.Empty : _diskLabel.Text, true);
        ...
        private static string FormatCompactCapacityFromBytes(double bytes)
        {
            if (bytes <= 0)
            {
                return string.Empty;
            ...
            if (gb >= 1024d)
            {
                return string.Format("{0:0.#}TB", gb / 1024d);
            }
            ...
            try
            {
                ...
                using (var logicalDiskSearcher = new ManagementObjectSearcher("SELECT Size, FreeSpace, DriveType FROM Win32_LogicalDisk WHERE DriveType = 3"))
                {
                    ...
                    else
                    {
                        ...
                        _memoryDisplayTotalBytes = GetMemoryDisplayTotalBytes(
                            memories,
                    ...
                    if (disks.Count == 0)
                    {
                        _diskDisplayTotalBytes = 0d;
                        _diskLabel.Text = "无数据";
                    }
                    else
                    {
                        _diskDisplayTotalBytes = GetDiskDisplayTotalBytes(disks, logicalDisks);
        ...
        {
            var displayTotalBytes = GetMemoryDisplayTotalBytes(memories, totalPhysicalBytes);
            if (memories == null || memories.Count == 0)
            {
                return FormatTotalWithActualUsable(displayTotalBytes, visiblePhysicalBytes);
            }
            ...
            return string.Format(
                "{0}，{1} 条\r\n{2}",
                FormatTotalWithActualUsable(displayTotalBytes, visiblePhysicalBytes),
                memories.Count,
                string.Join("\r\n", memoryParts));
        }
        ...
        private static string BuildDiskSummary(List<ManagementObject> disks, List<ManagementObject> logicalDisks)
        {
            ...
            var totalSize = GetDiskDisplayTotalBytes(disks, logicalDisks);
        ...
        private static double GetMemoryDisplayTotalBytes(List<ManagementObject> memories, double totalPhysicalBytes)
        {
            if (memories != null && memories.Count > 0)
            {
                var totalFromModules = memories
                    .Select(memory => ToDouble(memory["Capacity"]))
                    .Where(capacity => capacity > 0)
                    .Sum();
                if (totalFromModules > 0)
                {
                    return totalFromModules;
                }
            }

            return totalPhysicalBytes > 0 ? totalPhysicalBytes : 0d;
        }

        private static double GetDiskDisplayTotalBytes(List<ManagementObject> disks, List<ManagementObject> logicalDisks)
        {
            var logicalTotalBytes = logicalDisks == null
                ? 0d
                : logicalDisks.Sum(logicalDisk => ToDouble(logicalDisk["Size"]));
            var internalPhysicalTotalBytes = disks == null
                ? 0d
                : disks
                    .Where(disk => !IsExternalUsbDisk(
                        SafeToString(disk["Model"]),
                        SafeToString(disk["InterfaceType"]),
                        SafeToString(disk["PNPDeviceID"])))
                    .Sum(disk => ToDouble(disk["Size"]));
            var allPhysicalTotalBytes = disks == null ? 0d : disks.Sum(disk => ToDouble(disk["Size"]));

            if (logicalTotalBytes > 0 && internalPhysicalTotalBytes > 0)
            {
                return Math.Min(logicalTotalBytes, internalPhysicalTotalBytes);
            }

            if (logicalTotalBytes > 0)
            {
                return logicalTotalBytes;
            }

            if (internalPhysicalTotalBytes > 0)
            {
                return internalPhysicalTotalBytes;
            }

            return allPhysicalTotalBytes;
        }

        private static bool IsExternalUsbDisk(string model, string interfaceType, string pnpDeviceId)
        {
            var upperInterface = (interfaceType ?? string.Empty).ToUpperInvariant();
            var upperPnp = (pnpDeviceId ?? string.Empty).ToUpperInvariant();

            return upperInterface.Contains("USB")
                || upperPnp.StartsWith("USBSTOR", StringComparison.Ordinal)
                || upperPnp.Contains("USB");
        }
